...
/* Crear una instancia con timeout más largo para operaciones batch */
export const clienteApiLargo = axios.create({
  baseURL: URL_BASE,
  headers: { "Content-Type": "application/json" },
...
const requestInterceptor = (configuracion) => {
...
instanciaAxios.interceptors.request.use(requestInterceptor, (error) =>
  Promise.reject(error),
);

clienteApiLargo.interceptors.request.use(requestInterceptor, (error) =>
  Promise.reject(error),
);
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !peticionOriginal._reintento) {
      ...
      if (refreshToken) {
        try {
          if (!refrescando) {
            refrescando = instanciaAxios.post("/auth/refresh/", {
...
// Agregar el mismo interceptor de respuesta a clienteApiLargo
clienteApiLargo.interceptors.response.use(
  async (respuesta) => {
    try {
      if (respuesta.data && respuesta.data.ciphertext) {
        const descifrado = await descifrarPayload(
    ...
    if (error.response?.status === 401 && !peticionOriginal._reintento) {
      ...
      if (refreshToken) {
        try {
          ...
          localStorage.setItem("jwt", data.access);
          peticionOriginal.headers.Authorization = `Bearer ${data.access}`;
          return clienteApiLargo(peticionOriginal);
        } catch {
          refrescando = null;
          console.warn("Refresh token inválido — cerrando sesión");
        }
...
export const clienteApi = {
  ...
  async put(endpoint, datos, configuracion = {}) {
    try {
      const respuesta = await instanciaAxios.put(
        endpoint,
        datos,
        configuracion,
      );
  ...
  async patch(endpoint, datos, configuracion = {}) {
    try {
      const respuesta = await instanciaAxios.patch(
        endpoint,
        datos,
        configuracion,
      );
...
/* Cliente con timeout largo para operaciones batch */
export const clienteApiLargoWrapper = {
  /**
   * GET — descifra automáticamente
   */
  async get(endpoint, configuracion = {}) {
    try {
      const respuesta = await clienteApiLargo.get(endpoint, configuracion);
      return respuesta.data;
    } catch (error) {
      manejarError("GET", endpoint, error);
      throw error;
  ...
  async post(endpoint, datos, configuracion = {}) {
    try {
      const respuesta = await clienteApiLargo.post(
        endpoint,
        datos,
        configuracion,
      );
  ...
  async put(endpoint, datos, configuracion = {}) {
    try {
      const respuesta = await clienteApiLargo.put(
        endpoint,
        datos,
        configuracion,
      );
  ...
  async patch(endpoint, datos, configuracion = {}) {
    try {
      const respuesta = await clienteApiLargo.patch(
        endpoint,
        datos,
        configuracion,
      );
  ...
  async delete(endpoint, configuracion = {}) {
    try {
      const respuesta = await clienteApiLargo.delete(endpoint, configuracion);
      return respuesta.data;
    } catch (error) {
      manejarError("DELETE", endpoint, error);
      throw error;
...
function manejarError(metodo, endpoint, error) {
  if (error.response) {
    ...
    switch (estado) {
      case 400:
        console.error("  → Datos inválidos. Verifica los datos enviados.");
        break;
      case 401:
      ...
      case 403:
        if (mensaje.includes("Tampering") || mensaje.includes("HMAC")) {
          console.error(
            "  → ALERTA DE SEGURIDAD: ¡Los datos fueron modificados!",
          );
        } else {
      ...
      case 500:
        ...
        break;
      default:
        console.error(`  → Error HTTP ${estado}`);
    }
