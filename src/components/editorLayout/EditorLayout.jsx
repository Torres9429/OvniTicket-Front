...
import {
  ...
  updateZone,
  deleteZone,
} from '../../services/zonas.api';
import {
  createGridCell,
  deleteGridCell,
  getGridCellsByLayout,
} from '../../services/gridCells.api';
...
function ensureValidLayout(layout) {
  ...
  const sections = (normalized.sections || []).map((section, sectionIdx) => {
    if (Array.isArray(section.rows) && section.rows.length > 0) {
      const validRows = section.rows.every(
        (row) => Array.isArray(row.seats) && row.seats.length > 0
      );
...
function updateSectionShape(section, updates) {
  const numRows = Math.max(
    1,
    Number(updates.numRows ?? section.numRows ?? section.rows?.length ?? 1)
  );
...
export default function EditorLayout({
  venueId: idLugar,
  ownerId: idDueno,
  initialVenue: venueInicial = null,
  existingLayoutId: idLayoutExistente = null,
  initialLayout: layoutInicial = null,
  onSaved: onGuardado,
  onGoBack: onVolver = null,
}) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const venueData = venueInicial || (await getVenue(idLugar));
        if (cancelled) return;
        ...
        if (
          ...
          venueOwnerId != null &&
          String(venueOwnerId) !== String(resolvedOwnerId)
        ) {
        ...
        const initialLayoutId =
          resolvedInitialLayout?.id_layout ||
          resolvedInitialLayout?.id ||
          idLayoutExistente;
        ...
        if (loadedZones.length === 0) {
          ...
          loadedZones = (allZones || []).filter(
            (zona) => String(zona.id_layout) === String(initialLayoutId)
          );
        }
        ...
        if (
          snapshotData &&
          (snapshotData.sections?.length > 0 || snapshotData.elements?.length > 0)
        ) {
          snapshot = normalizeLayoutZones(snapshotData);
        } else {
          const currentLayoutId = layoutData.id_layout || initialLayoutId;
          const celdas = await getGridCellsByLayout(currentLayoutId).catch(() => []);
          if (celdas && celdas.length > 0) {
            snapshot = legacyGridToLayout({
              layout: layoutData,
              celdas,
              zonas: backendZones,
            });
        ...
        const mergedZones =
          (snapshot.zones || []).length > 0 ? snapshot.zones : backendZones;
        ...
        setEstadoLayout(
          String(
            layoutData.estatus || resolvedInitialLayout?.estatus || 'borrador'
          ).toLowerCase()
        );
        setHasUnsavedChanges(false);
      ...
      if (selectedItem.kind === 'section') {
        return {
          ...
          sections: (prev.sections || []).filter(
            (section) => section.id !== selectedItem.id
          ),
        };
      }
    ...
    (sectionId, transform) => {
      updateLayout((prev) => ({
        ...
        sections: (prev.sections || []).map((section) => {
          ...
          const oldSeatsPerRow =
            section.rows?.[0]?.seats?.length || section.seatsPerRow || 1;
          const nextRows = Math.max(1, Math.round(oldRows * transform.scaleY));
          const nextSeatsPerRow = Math.max(
            1,
            Math.round(oldSeatsPerRow * transform.scaleX)
          );
  ...
  const handleAssignSectionZone = useCallback(
    (sectionId, zoneId) => {
      updateLayout((prev) => ({
        ...prev,
        sections: (prev.sections || []).map((section) =>
  ...
  const handleSaveSectionDialog = useCallback(
    (payload) => {
  ...
  const getSelectedSection = useMemo(
    () =>
      layout.sections?.find((section) => section.id === editingSectionId) ||
      null,
    ...
    async ({ silencioso = false } = {}) => {
      ...
      try {
        ...
        if (!layoutId) {
          ...
          await apiUpdateLayout(layoutId, {
            grid_rows: snapshot.grid_rows,
            grid_cols: snapshot.grid_cols,
            version: normalizedLayout.version || 1,
            estatus: estadoLayout.toUpperCase(),
        ...
        const zonesForLayout = (existingZones || []).filter(
          (zone) => String(zone.id_layout) === String(layoutId)
        );
          ...
          (zonesForLayout || [])
            .filter(
              (zone) =>
                !zonesAfterSave.some(
                  (currentZone) =>
                    String(currentZone.idBackend) === String(zone.id_zona)
                )
            )
            .map((zone) => deleteZone(zone.id_zona).catch(() => {}))
        );

        const cells = layoutToGridSnapshot({
          ...normalizedLayout,
          zones: zonesAfterSave,
        }).cells;
        const existingCells = await getGridCellsByLayout(layoutId).catch(() => []);
        await Promise.all(
          (existingCells || []).map((cell) =>
            deleteGridCell(cell.id_grid_cells).catch(() => {})
          )
        );
        ...
        await saveLayoutSnapshot(layoutId, {
          layout_data: {
            ...normalizedLayout,
            zones: zonesAfterSave,
          },
        ...
        setLayout((prev) =>
          normalizeLayoutZones({ ...prev, zones: zonesAfterSave })
        );
        setHasUnsavedChanges(false);
  ...
  const handleSaveSnapshot = useCallback(async () => {
    ...
    try {
      ...
      setError(
        snapshotError?.response?.data?.error ||
  ...
  const handlePublishToggle = useCallback(async () => {
    ...
    try {
      const nextStatus =
        estadoLayout === 'publicado' ? 'borrador' : 'publicado';
      await apiUpdateLayout(layoutId, {
      ...
      setMensaje(
        nextStatus === 'publicado'
          ? 'Layout publicado correctamente'
          : 'Layout cambiado a borrador'
      );
    } catch (publishError) {
      setError(
        publishError?.response?.data?.error ||
          publishError.message ||
          'No se pudo cambiar el estado del layout.'
      );
    } finally {
      setGuardandoSnapshot(false);
    }
      ...
      {!error &&
        ...
        layout.elements.length === 0 && (
          <div className="rounded-xl bg-warning/10 px-4 py-3 text-sm text-warning">
            El layout está vacío. Agrega una sección o un elemento para empezar.
          </div>
        )}
